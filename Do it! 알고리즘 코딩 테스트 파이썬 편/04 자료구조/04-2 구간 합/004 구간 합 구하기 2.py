...
# 0 라인에 0을 채워넣는다면 코드가 더 깔끔하게 최적화 할 수 있음.
# NxN 개의 수
mapList = [[0] * (n + 1)]

for _ in range(n):
    mapList.append([0] + list(map(int, readline().split())))

# 이차원 구간 합 구하기
sum2ndList = [[0] * (n + 1) for _ in range(n + 1)]
...
for _ in range(quizNum):
    x1, y1, x2, y2 = map(int, readline().split())
    write(str(sum2ndList[x2][y2] - sum2ndList[x1 - 1][y2] - sum2ndList[x2][y1 - 1] + sum2ndList[x1 - 1][y1 - 1]) + '\n')