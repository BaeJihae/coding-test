...
# adj : 인접리스트, n : 노드의 개수
def dijkstra(adj, adj_op, n, s):
    # s -> x까지 노드의 최소 시간
    distance_lst = [float('inf')] * (n + 1)
    # x -> s까지 노드의 최소 시간
    distance_op_lst = [float('inf')] * (n + 1)
    # 시작 노드와 끝 노드 초기화
    distance_op_lst[s], distance_lst[s] = 0, 0

    min_heap = [(0, s)]  # (노드의 최소 거리, 시작 노드)
    min_heap_op = [(0, s)]  # (반대로 갈 때의 노드의 최소 거리, 시작 노드)
    ...
    # 각 노드에서 시작 노드까지의 최소 시간 구하기
    while min_heap_op:
        d, n = heappop(min_heap_op)
        for nxt_n, nxt_d in adj_op[n]:
            if distance_op_lst[nxt_n] > d + nxt_d:
                distance_op_lst[nxt_n] = d + nxt_d
                heappush(min_heap_op, (distance_op_lst[nxt_n], nxt_n))

    return distance_lst, distance_op_lst
...
def solution():
    for tc in range(1, int(input()) + 1):
        ...
        result_adj, result_adj_op = dijkstra(adj, adj_op, n, s)
